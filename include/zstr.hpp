...
{
public:
    z_stream_wrapper(bool _is_input = true, int _level = 9)
        : is_input(_is_input)
    {
        this->zalloc = Z_NULL;
        this->zfree = Z_NULL;
...
public:
    ostreambuf(std::streambuf * _sbuf_p,
               std::size_t _buff_size = default_buff_size, int _level = 9)
        : sbuf_p(_sbuf_p),
...
{
public:
    explicit ofstream(const std::string& filename,int expbuffer, int level, std::ios_base::openmode mode = std::ios_base::out)
        : detail::strict_fstream_holder< strict_fstream::ofstream >(filename, mode | std::ios_base::binary),
          std::ostream(new ostreambuf(_fs.rdbuf(),(std::size_t)1 << expbuffer,level))
    {
        exceptions(std::ios_base::badbit);
    }
    virtual ~ofstream()
